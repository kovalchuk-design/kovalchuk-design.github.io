...
(() => {
  ...
  // Writes go to the host by basename, which always lands at the project
  // root — so the read has to resolve there too, or a page in a
  // subdirectory (work/*.html) fetches a sidecar that isn't there and
  // silently loses every drop on reload. This script lives in assets/,
  // one level below the root, so its own URL gives us the root.
  const STATE_URL = (() => {
    try {
      const s = document.currentScript && document.currentScript.src;
      if (s) return new URL('../' + STATE_FILE, s).href;
    } catch (e) {}
    return STATE_FILE;
  })();
